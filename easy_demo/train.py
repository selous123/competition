...
def conv(input_data, w, b, stride=1, use_bn=False):
    y = tf.nn.bias_add(tf.nn.conv2d(input_data, w, strides=[1, stride, stride, 1], padding='SAME'), b)
    if use_bn:
...
INPUT = tf.placeholder(tf.float32, [None, IMAGE_HEIGHT, IMAGE_WIDTH, 3])
...
def model():
    ...
    # 输出层
    fully_conn_2 = dense(input_data=dropout_fully_conn_1, output_units=CATEGORY_COUNT, activation=None)

    return fully_conn_2


def train_model(learning_rate=0.001, batch_size=32, dropout_rate=0.2, val_steps=100):
    """
    训练模型
    ...
    with tf.Session() as sess:
        train_writer = tf.summary.FileWriter(
            os.path.join('summary', datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')),
            sess.graph)
        step = 0
...
if __name__ == '__main__':
    # 训练集
    train_set = DataSet("U:/rpc_easy/norm_data", "U:/rpc_easy/train.csv")
    # 验证集
    val_set = DataSet("U:/rpc_easy/norm_data", "U:/rpc_easy/val.csv")
